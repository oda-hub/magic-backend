...
def get_targets_dic(targets_dic):
    names_dict={}
    for td in targets_dic:
        target_names = [k for k in td if 'Tpname' in k]
        names_dict = {}
        target_names.sort()
        for ID, n in enumerate(target_names):
            l = [td[n]]
            if n.replace('pname', 'aname') in td:
                alias = td[n.replace('pname', 'aname')]
                if alias is not None:
                    l.extend(td[n.replace('pname', 'aname')].split(';'))
